...
All production callers use `SerializableSnapshot`: `catalog.Catalog.ddl` and
`exec.Engine.Begin` both hard-code it. `Snapshot` exists in the interface and
the conformance suite but currently has no non-test caller.
