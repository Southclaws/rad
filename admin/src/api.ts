...
// /execute accepts PIR programs, never bare LIR queries. Pick a statement
// name outside the query's local binding namespace so wrapping an arbitrary
// LIR document cannot introduce a name collision during program binding.
export function queryProgram(relation: unknown): QueryProgram {
  const bindings =
    relation !== null && typeof relation === "object" && !Array.isArray(relation)
      ? (relation as Record<string, unknown>).bindings
      : undefined;
  const bindingNames =
  ...
  let name = "query";
  while (Object.hasOwn(bindingNames, name)) name += "_";

  return { statements: [{ name, kind: "query", relation }] };
}

function execute(
  program: unknown,
  opts?: { showPlan?: boolean; dryRun?: boolean }
) {
  const p = new URLSearchParams();
  if (opts?.showPlan) p.set("show-plan", "true");
  if (opts?.dryRun) p.set("dry-run", "true");
  const qs = p.toString();
  return publicRequest<ExecuteResponse>(
    `/execute${qs ? `?${qs}` : ""}`,
    json("POST", program)
  );
}
...
export const publicAPI = {
  ...
  renameTable: (table: string, name: string) =>
    publicRequest<TableInfo>(
      `/tables/${encodeURIComponent(table)}`,
      json("PATCH", { name })
    ),
  deleteTable: (table: string) =>
    publicRequest<void>(`/tables/${encodeURIComponent(table)}`, json("DELETE")),
  ...
  renameColumn: (table: string, column: string, name: string) =>
    publicRequest<TableInfo>(
      `/tables/${encodeURIComponent(table)}/columns/${encodeURIComponent(
        column
      )}`,
      json("PATCH", { name })
    ),
  ...
  createIndex: (table: string, index: IndexInfo) =>
    publicRequest<TableInfo>(
      `/tables/${encodeURIComponent(table)}/indexes`,
      json("POST", index)
    ),
  deleteIndex: (table: string, index: string) =>
    publicRequest<TableInfo>(
      `/tables/${encodeURIComponent(table)}/indexes/${encodeURIComponent(
        index
      )}`,
      json("DELETE")
    ),
  query: (relation: unknown) => execute(queryProgram(relation)),
