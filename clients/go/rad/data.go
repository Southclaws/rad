...
func (c *Client) tableInfo(ctx context.Context, table string) (protocol.TableInfo, error) {
	...
	if !c.schema.loaded {
		...
		for _, table := range tables {
			c.schema.byName[table.Name] = table
		}
		c.schema.loaded = true
	}
	return c.schema.table(table)
}

func (s *schemaCache) table(name string) (protocol.TableInfo, error) {
	info, ok := s.byName[name]
	if !ok {
		return protocol.TableInfo{}, fmt.Errorf("rad: unknown table %q", name)
	}
	return info, nil
}
