// Package rad is Rad's Go client runtime: a thin client for the rad://
// protocol built on the generated transport in clients/go/api/oas. Generated
// clients (rad generate) build on it; it can also be used directly for
// dynamic access.
//
//	c, err := rad.Dial("rad://localhost")
...
// gate caused by an online schema worker reaching its durable delta limit.
// Unlike a serializable conflict, callers should back off while the worker
// catches up instead of immediately retrying a hot loop.
func IsSchemaTransitionBackpressure(err error) bool {
	var ae *APIError
	return errors.As(err, &ae) && ae.Problem.Reason == "schema_transition_backpressure"
}

// IsSchemaTransitionFinalizing reports the short affected-table write gate
// used while an online unique index proves and publishes its final state.
func IsSchemaTransitionFinalizing(err error) bool {
	var ae *APIError
	return errors.As(err, &ae) && ae.Problem.Reason == "schema_transition_finalizing"
}
