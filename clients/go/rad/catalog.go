...
import (
	...
	"github.com/Southclaws/rad/clients/go/api"
...
// WaitSchemaMigration waits for every durable transition returned by a
// converging schema apply, then verifies that the current canonical schema
// reached the desired hash. Waiting is a client concern: the apply request and
// every worker batch use short independent transactions.
func (c *Client) WaitSchemaMigration(
	ctx context.Context,
	migration protocol.SchemaMigration,
) (protocol.SchemaMigration, error) {
	if migration.State == protocol.SchemaMigrationReady {
		return migration, nil
	}
	if migration.State != protocol.SchemaMigrationConverging || len(migration.TransitionIDs) == 0 {
		return protocol.SchemaMigration{}, fmt.Errorf(
			"rad: migration state %q has no observable convergence work",
			migration.State,
		)
	}
	...
	for {
		allReady := true
		for _, transitionID := range migration.TransitionIDs {
			transition, err := c.SchemaTransition(ctx, transitionID)
			if err != nil {
				return protocol.SchemaMigration{}, err
			}
			switch transition.State {
			case TransitionReady:
			case TransitionFailed, TransitionCancelled:
				return protocol.SchemaMigration{}, fmt.Errorf(
					"rad: schema transition %q ended in state %q: %s",
					transitionID,
					transition.State,
		...
		if allReady {
			state, err := c.Schema(ctx)
			if err != nil {
				return protocol.SchemaMigration{}, err
			}
			if state.SchemaHash != migration.DesiredHash {
				return protocol.SchemaMigration{}, fmt.Errorf(
					"rad: schema transitions published but current hash is %s, want %s",
					state.SchemaHash,
					migration.DesiredHash,
				)
			}
			migration.SchemaState = state
			migration.State = protocol.SchemaMigrationReady
			c.schema.invalidate()
			return migration, nil
		}
		select {
		case <-ctx.Done():
			return protocol.SchemaMigration{}, ctx.Err()
		case <-ticker.C:
		}
	}
}
...
// transition without applying it. Matching durable work may make the program
// omit duplicate starts; SchemaMigrate replans and returns the authoritative
// transition identities accepted by apply.
...
func (c *Client) SchemaMigrate(
	...
	case *oas.SchemaMigrateResult:
		migration := protocol.SchemaMigration{
			...
			DesiredHash: value.DesiredHash, State: protocol.SchemaMigrationState(value.State),
			TransitionIDs: value.TransitionIds,
