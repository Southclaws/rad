...
const (
	ProblemStageSchema    = "schema"
	ProblemStagePreflight = "preflight"
	ProblemStageBinding   = "binding"
	ProblemStagePlanning  = "planning"
	ProblemStageExecution = "execution"
	ProblemStageStorage   = "storage"
)
...
type Problem struct {
	...
	// Stage identifies the engine boundary that rejected or failed the
	// operation. Rich locations and per-class context are carried by the
	// generated transport union; this compatibility type retains the common
	// fields used by the Go client.
...
func NewProblem(code string, status int, detail string) Problem {
	...
	return Problem{
		...
		Code:   code,
		Reason: code,
		Stage:  defaultProblemStage(code),
	}
}

func defaultProblemStage(code string) string {
	switch code {
	case CodeExecutionFailed:
		return ProblemStageExecution
	case CodeConflict:
		return ProblemStageStorage
	case CodeInternal:
		return ""
	default:
		return ProblemStagePreflight
	}
}
