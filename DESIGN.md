...
Mood, in one phrase: *a phosphor-green terminal glowing in a dark room at 2am
— clean, precise, quietly hacker-romantic.* The surface IS part of the brand
...
--surface:    oklch(0.205 0.010 162);  /* terminal windows, panels */
--surface-2:  oklch(0.245 0.012 162);  /* raised / inset within panels */
--line:       oklch(0.30 0.012 160);   /* hairline borders */
--line-2:     oklch(0.40 0.015 160);   /* stronger borders, focus rings base */
...
--green-deep: oklch(0.30 0.060 155);   /* tint fills behind text */
...
--amber-mid:  oklch(0.80 0.13 72);     /* amber on dark text contexts */
...
- **Spline Sans Mono** (400/500/700) — *primary voice.* Wordmark, hero
...
- **Hanken Grotesk** (400/500/600) — *prose counterweight.* Body paragraphs
...
- **The loop** — the define→run→generate→write→evolve→regenerate sequence is
  a *real ordered flow*, so numbering it is earned information, not `01/02/03`
