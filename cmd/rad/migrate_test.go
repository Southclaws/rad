...
func TestMigrateCmdSendsSchemaToServer(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/schema/migrate":
			...
			fmt.Fprintf(w, `{"schema_version":1,"schema_hash":%q,"schema":{"tables":[{"id":1,"name":"accounts","columns":[{"id":1,"name":"id","type":"text"}],"primary_key":["id"]}]},"changes":[{"kind":"create_table","summary":"create table accounts","table":"accounts"}],"desired_hash":%q,"state":"ready","transition_ids":[]}`, hash, hash)
		default:
