...
func schemaMigrateCmd(options *schemaOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if migration.State == protocol.SchemaMigrationConverging {
				fmt.Fprintf(
					cmd.OutOrStdout(),
					"Waiting for %d online schema transition(s)...\n",
					len(migration.TransitionIDs),
				)
				migration, err = client.WaitSchemaMigration(cmd.Context(), migration)
				if err != nil {
					return err
				}
			}
