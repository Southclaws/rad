...
func TestUpdateRewritesIndexes(t *testing.T) {
	...
	_, found, err := eng.Update(ctx, "users", lir.Row{"id": lir.Int64(1)}, lir.Row{"name": lir.Text("Alicia")})
	if err != nil || !found {
		t.Fatalf("found=%v err=%v", found, err)
	}

	row, _, _ := eng.GetByPrimaryKey(ctx, "users", lir.Row{"id": lir.Int64(1)})
	if !row["name"].Equal(lir.Text("Alicia")) {
		t.Fatalf("update lost: %v", row)
	}
	// The old index entry is gone, the new one serves.
	if rows, _ := eng.ScanIndex(ctx, "users", "users_name_idx", lir.Row{"name": lir.Text("Alice")}); len(rows) != 0 {
		t.Fatalf("stale index entry: %v", rows)
	}
	if rows, _ := eng.ScanIndex(ctx, "users", "users_name_idx", lir.Row{"name": lir.Text("Alicia")}); len(rows) != 1 {
...
func TestUpdateValidation(t *testing.T) {
	...
	if _, _, err := eng.Update(ctx, "users", lir.Row{"id": lir.Int64(1)}, lir.Row{"id": lir.Int64(9)}); err == nil {
		t.Error("primary key update accepted")
	}
	if _, _, err := eng.Update(ctx, "users", lir.Row{"id": lir.Int64(1)}, lir.Row{"ghost": lir.Int64(1)}); err == nil {
	...
	if _, found, err := eng.Update(ctx, "users", lir.Row{"id": lir.Int64(404)}, lir.Row{"name": lir.Text("x")}); err != nil || found {
...
func TestUpdateForeignKeyAndUniqueChecks(t *testing.T) {
	eng, ctx := setup(t)
	for i := int64(1); i <= 2; i++ {
		if err := eng.Insert(ctx, "users", lir.Row{"id": lir.Int64(i), "name": lir.Text("u")}); err != nil {
			t.Fatal(err)
		}
	}
	if err := eng.Insert(ctx, "orders", lir.Row{"id": lir.Int64(10), "user_id": lir.Int64(1)}); err != nil {
		t.Fatal(err)
	}

	// Repointing an FK is validated.
	if _, _, err := eng.Update(ctx, "orders", lir.Row{"id": lir.Int64(10)}, lir.Row{"user_id": lir.Int64(2)}); err != nil {
		t.Fatalf("valid fk repoint: %v", err)
	}
	if _, _, err := eng.Update(ctx, "orders", lir.Row{"id": lir.Int64(10)}, lir.Row{"user_id": lir.Int64(42)}); err == nil {
	...
	_ = eng.Insert(ctx, "handles", lir.Row{"id": lir.Int64(1), "handle": lir.Text("a")})
	_ = eng.Insert(ctx, "handles", lir.Row{"id": lir.Int64(2), "handle": lir.Text("b")})
	if _, _, err := eng.Update(ctx, "handles", lir.Row{"id": lir.Int64(2)}, lir.Row{"handle": lir.Text("a")}); err == nil {
		t.Error("unique violation via update accepted")
	}
	// A no-op rewrite of the same value is fine (it points at the same PK).
	if _, _, err := eng.Update(ctx, "handles", lir.Row{"id": lir.Int64(2)}, lir.Row{"handle": lir.Text("b")}); err != nil {
...
func TestDeleteRemovesRowAndIndexEntries(t *testing.T) {
	...
	found, err := eng.Delete(ctx, "users", lir.Row{"id": lir.Int64(1)})
	if err != nil || !found {
		t.Fatalf("found=%v err=%v", found, err)
	}
	...
	if found, err := eng.Delete(ctx, "users", lir.Row{"id": lir.Int64(1)}); err != nil || found {
...
func TestDeleteRestrict(t *testing.T) {
	...
	_, err := eng.Delete(ctx, "users", lir.Row{"id": lir.Int64(1)})
	...
	if _, err := eng.Delete(ctx, "orders", lir.Row{"id": lir.Int64(10)}); err != nil {
		t.Fatal(err)
	}
	if found, err := eng.Delete(ctx, "users", lir.Row{"id": lir.Int64(1)}); err != nil || !found {
...
func TestDeleteRestrictSelfReferential(t *testing.T) {
	...
	_ = eng.Insert(ctx, "nodes", lir.Row{"id": lir.Int64(1)})
	_ = eng.Insert(ctx, "nodes", lir.Row{"id": lir.Int64(2), "parent_id": lir.Int64(1)})

	if _, err := eng.Delete(ctx, "nodes", lir.Row{"id": lir.Int64(1)}); err == nil {
		t.Fatal("deleted a node with children")
	}
	if _, err := eng.Delete(ctx, "nodes", lir.Row{"id": lir.Int64(2)}); err != nil {
		t.Fatalf("leaf delete: %v", err)
	}
	if found, err := eng.Delete(ctx, "nodes", lir.Row{"id": lir.Int64(1)}); err != nil || !found {
