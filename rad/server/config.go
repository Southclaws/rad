...
//	RAD_CATALOG_MODE direct | schema          (fresh databases only; default "direct")
...
type Config struct {
	...
	// CatalogMode is the requested catalog management mode (direct |
	// schema). It applies only when initialising a fresh database — the
	// mode is set once and stored in the catalog; on an existing database
	// the stored mode wins and an explicit mismatch is a startup error.
	// Empty means no preference (fresh databases default to direct).
	CatalogMode string
