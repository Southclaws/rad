...
func (s *Server) snapshot(ctx context.Context) (*sql.Schema, error) {
	...
	for _, t := range tables {
		...
		for _, col := range t.Columns {
			...
			if col.InsertDefault != nil {
				ci.Default = &protocol.ColumnDefault{Func: string(col.InsertDefault.Func)}
			}
...
func (s *Server) executeLocked(ctx context.Context, state *sessionTransaction, writer wire.DataWriter, c *sql.Compiled) error {
	...
	if state.tx != nil {
		...
		// The shared program executor gives read-only programs a snapshot
		// fast path. A write is still one atomic program, so a lost
		// serializable race can be replayed invisibly — the behavior clients
		// expect from Postgres single-statement writes.
