...
func statementToEngine(s pirwire.Statement) (execprogram.Statement, error) {
	...
	case *pirwire.ChangeColumnDefaultStatement:
		input, err := pirDefaultInput(x.Default)
		if err != nil {
			return execprogram.Statement{}, fmt.Errorf("statement %q: %w", x.Name, err)
		}
		return execprogram.Statement{
			Name: x.Name, Kind: execprogram.ChangeColumnDefault,
			TableID: schemaID(x.TableID), ColumnID: schemaID(x.ColumnID),
			InsertDefault: input,
		}, nil
	...
	case *pirwire.StartIndexBuildStatement:
		return execprogram.Statement{
			Name: x.Name, Kind: execprogram.StartIndexBuild,
			TableID: schemaID(x.TableID), Index: pirIndexDef(x.Index),
			Prerequisites: x.Prerequisites,
			After:         x.After,
		}, nil
	case *pirwire.StartColumnReplacementStatement:
		replacement, err := pirColumnReplacementDef(x.Replacement)
		if err != nil {
			return execprogram.Statement{}, fmt.Errorf("statement %q: %w", x.Name, err)
		}
		return execprogram.Statement{
			Name: x.Name, Kind: execprogram.StartColumnReplacement,
			TableID: schemaID(x.TableID), ColumnID: schemaID(x.ColumnID),
	...
	case *pirwire.StartConstraintValidationStatement:
		return execprogram.Statement{
			Name: x.Name, Kind: execprogram.StartConstraintValidation,
			TableID:    schemaID(x.TableID),
			Constraint: pirConstraintDef(x.Constraint),
			After:      x.After,
		}, nil
	default:
...
func programResult(res execprogram.Result, showPlan, dryRun bool) (oas.ExecuteRes, error) {
	...
	for i, s := range res.Statements {
		stmts[i] = oas.StatementResult{Name: s.Name, Affected: s.Affected, Control: oas.Value([]byte("null"))}
		if s.Control != nil {
			raw, err := json.Marshal(s.Control)
			if err != nil {
				return nil, fmt.Errorf("encode statement %q control result: %w", s.Name, err)
			}
			stmts[i].Control = oas.Value(raw)
		}
	}
