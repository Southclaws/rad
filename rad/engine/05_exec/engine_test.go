...
func setupWithOptions(t *testing.T, options ...Option) (*Engine, context.Context) {
	t.Helper()
	...
	options = append([]Option{withAutomaticIndexBuilds(false)}, options...)
	eng := New(store, cat, options...)
...
func openFileEngine(t *testing.T, databaseName string, options ...Option) (*kvslate.Store, *Engine) {
	t.Helper()
	database, err := kvslate.Open(databaseName, "file://")
	if err != nil {
		t.Fatal(err)
	}
	engine := New(database, catalog.New(database), options...)
	t.Cleanup(func() {
		_ = engine.Close()
		_ = database.Close()
	})
...
func TestSchemaJobDeltaLimitNormalizationCannotOverflow(t *testing.T) {
	for _, test := range []struct {
		name   string
		config SchemaJobConfig
	}{
		{name: "soft only", config: SchemaJobConfig{DeltaSoftLimit: ^uint64(0)}},
		{name: "equal soft and hard", config: SchemaJobConfig{
			DeltaSoftLimit: ^uint64(0), DeltaHardLimit: ^uint64(0),
		}},
	} {
		t.Run(test.name, func(t *testing.T) {
			engine, _ := setupWithOptions(t, WithSchemaJobConfig(test.config))
			if engine.schemaJobConfig.DeltaHardLimit == 0 ||
				engine.schemaJobConfig.DeltaHardLimit <= engine.schemaJobConfig.DeltaSoftLimit {
				t.Fatalf("normalized delta limits = soft %d hard %d",
					engine.schemaJobConfig.DeltaSoftLimit,
					engine.schemaJobConfig.DeltaHardLimit,
				)
			}
		})
	}
