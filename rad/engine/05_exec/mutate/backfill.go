...
func BackfillIndex(ctx context.Context, view kv.KV, table model.Table, index model.Index) error {
	...
	for {
		...
		columns := table.IndexColumnNames(index)
		tuple, err := codec.EncodeRowTuple(row, columns)
		if err != nil {
			return err
		}
		if index.Unique && !anyNullComponent(row, columns) {
			if previous, duplicate := seen[string(tuple)]; duplicate {
				return reject.Fail(reject.ReasonConstraintViolation,
					"exec: cannot backfill unique index %q: rows %v and %v share a value", index.Name, previous, row)
			}
