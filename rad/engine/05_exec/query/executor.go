...
func New(view kv.KV, recur Limits) *Executor {
	return &Executor{
		view:     view,
		bindings: map[string][]lireval.Env{},
		frontier: map[string][]lireval.Env{},
...
func (ex *Executor) commit(ctx context.Context, bindings []planner.BindingPlan) error {
	...
	for _, binding := range bindings {
		...
		op, err := ex.build(ctx, binding.Plan, lireval.Env{})
		if err != nil {
			return err
		}
...
func (ex *Executor) build(ctx context.Context, node planner.PhysNode, outer lireval.Env) (operator, error) {
	switch n := node.(type) {
	case *planner.PKGetExec:
		return ex.buildPKGet(ctx, n, outer)
	case *planner.TableScanExec:
		it, err := rowstore.ScanTableColumns(ctx, ex.view, n.Scan.Table, n.DecodeColumns)
		if err != nil {
			return nil, err
		}
	...
	case *planner.RefExec:
		...
		in, err := ex.build(ctx, binding.Plan, lireval.Env{})
		if err != nil {
			return nil, err
		}
