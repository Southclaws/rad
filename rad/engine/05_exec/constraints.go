...
func checkForeignKeysFor(ctx context.Context, view kv.KV, tbl catalog.Table, row, set lir.Row) error {
	for _, fk := range tbl.ForeignKeys {
...
func checkForeignKey(ctx context.Context, view kv.KV, row lir.Row, fk catalog.ForeignKey) error {
	...
	parentTuple, err := EncodeTuple(values)
	if err != nil {
		return err
	}
...
func checkUniqueIndex(ctx context.Context, view kv.KV, tbl catalog.Table, row lir.Row, pkTuple []byte, idx catalog.Index) error {
...
func checkNoReferences(ctx context.Context, view kv.KV, tbl catalog.Table, row lir.Row) error {
	tables, err := catalog.NewReader(view).ListTables(ctx)
	if err != nil {
		return err
	}
...
func anyRowMatching(ctx context.Context, view kv.KV, child catalog.Table, columns []string, want lir.Row, parent catalog.Table, parentRow lir.Row) (bool, error) {
	var selfPK []byte
	if child.ID == parent.ID {
		var err error
		selfPK, err = encodeRowTuple(parentRow, parent.PrimaryKey)
		if err != nil {
			return false, err
		}
	}
	...
	for _, idx := range child.Indexes {
		...
		prefixTuple, err := encodeRowTuple(want, columns)
		if err != nil {
			return false, err
		}
		prefix := append(IndexPrefix(child.ID, idx.ID), prefixTuple...)
		it, err := view.Scan(ctx, prefix, keyenc.PrefixEnd(prefix))
		if err != nil {
			return false, err
		}
	...
	for {
		...
		if selfPK != nil {
			pk, err := encodeRowTuple(candidate, child.PrimaryKey)
			if err != nil {
				return false, err
			}
