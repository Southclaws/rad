...
func extract(t *testing.T, root lir.Relation) *analysis.ScanConstraints {
	...
	cs := analysis.ExtractConstraints(q.Root)
	if cs == nil {
...
func TestExtractOuterEquality(t *testing.T) {
	...
	cs := analysis.ExtractConstraints(owner.Rel)
	if cs == nil {
...
func TestClassifyForcingQuery(t *testing.T) {
	...
	oc := analysis.Classify(owner.Rel)
	if oc.Kind != analysis.KeyCorrelated || len(oc.Keys) != 1 ||
	...
	tc := analysis.Classify(tasks.Rel)
	if tc.Kind != analysis.KeyCorrelated || len(tc.Keys) != 1 ||
	...
	cc := analysis.Classify(count.Rel)
	if cc.Kind != analysis.KeyCorrelated || len(cc.Keys) != 1 ||
		cc.Keys[0].InnerCol != "task_id" {
		t.Fatalf("comment-count correlation = %+v", cc)
	}

	// And an uncorrelated relation classifies as such.
	if got := analysis.Classify(q.Root); got.Kind != analysis.Uncorrelated {
...
func TestClassifyGeneral(t *testing.T) {
	...
	if got := analysis.Classify(arr.Rel); got.Kind != analysis.GeneralCorrelated {
