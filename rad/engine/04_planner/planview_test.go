...
func TestPlanViewAccessDecision(t *testing.T) {
	...
	view := planner.NewPlanView(planner.PlanQuery(q))
...
func TestPlanViewNoAccessNoise(t *testing.T) {
	...
	if s := planner.NewPlanView(planner.PlanQuery(q)).String(); strings.Contains(s, "access:") {
		t.Fatalf("unconstrained scan should render no access decision:\n%s", s)
	}
}

func findOp(n *planner.PlanNodeView, op string) *planner.PlanNodeView {
	if n == nil {
		return nil
	}
	if n.Op == op {
