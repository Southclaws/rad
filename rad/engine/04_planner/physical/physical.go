...
type PhysPlan struct {
	...
	Card         lir.RootCard
	Out          lir.RowType
	Dependencies model.CatalogDependencies
...
type PKGetExec struct {
	Scan          *bound.Scan
	Key           []analysis.ConstVal // one per primary-key column, in key order
	DecodeColumns []model.Column
...
type IndexRangeScanExec struct {
	Scan          *bound.Scan
	Index         model.Index
	EqPrefix      []analysis.ConstVal // one per leading index column
	Range         *RangeSpec
	DecodeColumns []model.Column
	Access        *AccessDecision
}
