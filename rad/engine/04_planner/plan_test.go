...
func planOf(t *testing.T, root lir.Relation) string {
	...
	return explain.PrintPlan(planner.PlanQuery(q))
}
...
func TestPlanForcingQueryGolden(t *testing.T) {
	...
	got := explain.PrintPlan(planner.PlanQuery(q))
...
func TestPlanBindingStrategy(t *testing.T) {
	...
	want := map[string]physical.BindingStrategy{
		"once":  physical.BindingReplay,
		"twice": physical.BindingMaterialise,
	}
