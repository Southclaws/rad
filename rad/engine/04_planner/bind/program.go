...
func (p *ProgramBinder) Bind(s ProgramStmt) (BoundStatement, error) {
	...
	if s.Mutation {
		schema, table, err := p.b.tableSchema(s.Table)
		if err != nil {
			return BoundStatement{}, statementErr(s.Name, err)
		}
		resultRoot = schema
		resultOut = schema.Output()
		pp.Dependencies.AddTableWrite(table)
		target = &table
	}
	p.program[s.Name] = &bound.Binding{Name: s.Name, Root: resultRoot}
	p.next++
	return BoundStatement{
		Name: s.Name, Plan: pp, ResultOut: resultOut, ResultCard: resultCard, Target: target,
	}, nil
}
...
func (b *binder) tableSchema(table string) (bound.Relation, model.Table, error) {
	if table == "" {
		return nil, model.Table{}, reject.Inputf("planner: mutation statement needs a target table")
	}
	tbl, ok, err := b.cat.GetTable(b.ctx, table)
	if err != nil {
		return nil, model.Table{}, err
	}
	if !ok {
		return nil, model.Table{}, reject.Fail(reject.ReasonUnknownTable, "planner: unknown table %q", table)
	}
	return bound.NewScan(tbl, "", b.freshSlots(len(tbl.Columns))), tbl, nil
}
