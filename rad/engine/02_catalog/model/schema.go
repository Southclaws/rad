...
func BuildSchema(tables []Table) (Schema, error) {
	...
	for _, table := range tables {
		...
		for _, column := range table.Columns {
			...
			if column.MissingValue != nil && column.MissingValue.Func != "" {
				return Schema{}, reject.Fail(
					reject.ReasonCatalogDrift,
					"catalog: physical column %q.%q has generator %q as its historical missing value",
					table.Name,
					column.Name,
					column.MissingValue.Func,
				)
			}
			...
			if column.InsertDefault != nil {
				copy := *column.InsertDefault
				defaultValue = &copy
			}
