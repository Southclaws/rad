...
func TestChangeColumnInsertDefaultPreservesMissingValue(t *testing.T) {
	cat, _, ctx := newCatalog(t)
	if _, err := cat.CreateTable(ctx, usersDef()); err != nil {
		t.Fatal(err)
	}
	initial := &model.Default{Text: "active"}
	table, err := cat.CreateColumn(ctx, "users", model.ColumnDef{
		Name: "status", Type: model.TypeText, Nullable: true, Default: initial,
	})
	if err != nil {
		t.Fatal(err)
	}
	initial.Text = "caller-mutated"
	status, _ := table.Column("status")
	if status.InsertDefault == nil || status.InsertDefault.Text != "active" ||
		status.MissingValue == nil || status.MissingValue.Text != "active" {
		t.Fatalf("created status semantics = %+v", status)
	}
	originalProtocol := table.WriteProtocolGeneration

	next := &model.Default{Text: "pending"}
	table, err = cat.ChangeColumnInsertDefault(ctx, "users", "status", next)
	if err != nil {
		t.Fatal(err)
	}
	next.Text = "caller-mutated-again"
	status, _ = table.Column("status")
	if status.InsertDefault == nil || status.InsertDefault.Text != "pending" {
		t.Fatalf("insert default = %+v, want pending", status.InsertDefault)
	}
	if status.MissingValue == nil || status.MissingValue.Text != "active" {
		t.Fatalf("missing value changed with insert policy: %+v", status.MissingValue)
	}
	if table.WriteProtocolGeneration != originalProtocol+1 {
		t.Fatalf(
			"write protocol generation = %d, want %d",
			table.WriteProtocolGeneration,
			originalProtocol+1,
		)
	}

	same, err := cat.ChangeColumnInsertDefault(
		ctx,
		"users",
		"status",
	...
	if same.WriteProtocolGeneration != table.WriteProtocolGeneration {
		t.Fatalf(
			"no-op default change advanced protocol %d -> %d",
			table.WriteProtocolGeneration,
			same.WriteProtocolGeneration,
		)
	}

	generated, err := cat.ChangeColumnInsertDefault(
		ctx,
		"users",
		"status",
	...
	status, _ = generated.Column("status")
	if status.InsertDefault == nil || status.InsertDefault.Func != model.DefaultUUID ||
		status.MissingValue == nil || status.MissingValue.Text != "active" {
		t.Fatalf("generator change corrupted column semantics: %+v", status)
	}
	if _, err := cat.ChangeColumnInsertDefault(
		ctx,
		"users",
		"status",
		&model.Default{Func: model.DefaultNowMS},
	); err == nil || !strings.Contains(err.Error(), "int64") {
		t.Fatalf("wrong-type generator accepted: %v", err)
	}

	cleared, err := cat.ChangeColumnInsertDefault(ctx, "users", "status", nil)
	if err != nil {
		t.Fatal(err)
	}
	status, _ = cleared.Column("status")
	if status.InsertDefault != nil || status.MissingValue == nil ||
		status.MissingValue.Text != "active" {
		t.Fatalf("clearing insert default changed history: %+v", status)
	}
}
