...
func SaveTable(ctx context.Context, view kv.KV, table model.Table) error {
	if currentRaw, ok, err := view.Get(ctx, TableKey(table.ID)); err != nil {
		return err
	} else if ok {
		var current model.Table
		if err := json.Unmarshal(currentRaw, &current); err != nil {
			return fmt.Errorf("catalog: corrupt table entry %q: %w", TableKey(table.ID), err)
		}
		if table.DefinitionGeneration <= current.DefinitionGeneration {
			table.DefinitionGeneration = current.DefinitionGeneration + 1
		}
	} else if table.DefinitionGeneration == 0 {
		table.DefinitionGeneration = 1
	}
