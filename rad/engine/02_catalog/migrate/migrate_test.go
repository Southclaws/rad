...
`)
	...
	want := []string{
		...
		"start index build boards_user_id_idx on boards",
...
func TestColumnDefaultChangeByID(t *testing.T) {
	initial := `
tables:
  - id: 1
    name: items
...
`
	steps := diff(t, currentFrom(t, initial), `
tables:
  - id: 1
    name: items
    columns:
...
`)
	got := stepStrings(steps)
	if len(got) != 1 || got[0] != "change column default items.status" {
		t.Fatalf("default change: %v", got)
	}
...
tables:
  - id: 1
    ...
    columns:
      - { id: 1, name: id,    type: int64, pk: true }
      - { id: 2, name: state, type: string, nullable: true }
`)
	got = stepStrings(steps)
	want := []string{
		"rename table items -> inventory",
		"rename column inventory.status -> state",
		"change column default inventory.state",
	}
	if len(got) != len(want) {
		t.Fatalf("renamed default change: got %v, want %v", got, want)
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("renamed default change: got %v, want %v", got, want)
...
func TestColumnSemanticChangesBecomeOnlineTransitions(t *testing.T) {
	current := currentFrom(t, `
tables:
  - id: 1
    name: events
...
`)
	...
	typeChange := stepStrings(diff(t, current, `
tables:
  - id: 1
    name: events
    columns:
...
`))
	if len(typeChange) != 1 || typeChange[0] != "replace column events.value" {
		t.Fatalf("type change = %v", typeChange)
	}

	notNull := stepStrings(diff(t, current, `
tables:
  - id: 1
    name: events
    columns:
...
`))
	if len(notNull) != 1 || notNull[0] != "validate not-null events.value" {
		t.Fatalf("not-null change = %v", notNull)
	}
}

func TestColumnReplacementRebuildsRetainedIndexes(t *testing.T) {
	current := currentFrom(t, `
tables:
  - id: 1
    name: events
    columns:
      ...
      - { id: 3, name: category, type: string, nullable: true }
    indexes:
      - { name: events_value_category_uq, columns: [value, category], unique: true }
`)
...
`))
	want := []string{
		"delete index events_value_category_uq on events",
		"delete index events_value_idx on events",
		"replace column events.value",
		"start index build events_value_category_uq on events",
