...
// same migration.
//
// Physical column changes and historical constraint validation become durable
// transition starts. Unsupported representation dependencies produce errors
// instead of destructive guesses.
package migrate
...
// tables (dependency-ordered), created columns, insert-default changes, index
// changes, and deletes last (table deletes referencing-table-first, mirroring
// creation order).
type Step interface {
	step()
	String() string
...
func (ChangeColumnDefault) step() {}
func (CreateIndex) step()         {}
func (ReplaceColumn) step()       {}
func (ValidateNotNull) step()     {}
...
func (s ChangeColumnDefault) String() string {
	return fmt.Sprintf("change column default %s.%s", s.Table, s.Column)
}

func (s CreateIndex) String() string {
	return fmt.Sprintf("start index build %s on %s", s.Def.Name, s.Table)
}

func (s ReplaceColumn) String() string {
	return fmt.Sprintf("replace column %s.%s", s.Table, s.Column)
}

func (s ValidateNotNull) String() string {
	return fmt.Sprintf("validate not-null %s.%s", s.Table, s.Column)
}
...
func Diff(current []model.Table, desired *schema.Schema) ([]Step, error) {
	var renames, adds, defaultChanges, indexDeletes, replacements, constraints, indexCreates, columnDeletes, tableDeletes []Step
	...
	for _, d := range desired.Tables {
		...
		defaultChanges = append(defaultChanges, steps.defaultChanges...)
		indexDeletes = append(indexDeletes, steps.indexDeletes...)
		replacements = append(replacements, steps.replacements...)
		constraints = append(constraints, steps.constraints...)
...
type tableSteps struct {
	renames, adds, defaultChanges, indexDeletes, replacements, constraints, indexCreates, columnDeletes []Step
}
...
func diffTable(
	...
	for _, dc := range d.Def.Columns {
		...
		defaultChanged := (c.InsertDefault == nil) != (dc.Default == nil) ||
			c.InsertDefault != nil && *c.InsertDefault != *dc.Default
		physicalChange := c.Type != dc.Type || c.Format != dc.Format || !c.Nullable && dc.Nullable
		if physicalChange {
			out.replacements = append(out.replacements, ReplaceColumn{
				Table: name, Column: dc.Name, ColumnID: dc.ID,
				Def: model.ColumnReplacementDef{
					Type: dc.Type, Nullable: dc.Nullable, Format: dc.Format,
					Default: cloneDefault(dc.Default), Conversion: model.ColumnConversionStrictBuiltin,
				},
			})
			continue
		}
		if c.Nullable && !dc.Nullable {
			if defaultChanged {
				out.defaultChanges = append(out.defaultChanges, ChangeColumnDefault{
					Table: name, Column: dc.Name, Default: cloneDefault(dc.Default),
				})
			}
			out.constraints = append(out.constraints, ValidateNotNull{
				Table: name, Column: dc.Name,
				Def: model.ConstraintDef{
					Name: naming.NotNullConstraint(name, dc.Name), Kind: model.ConstraintNotNull, ColumnID: dc.ID,
				},
			})
			continue
		}
	...
	for _, idx := range cur.Indexes {
		// Planner-invisible transition definitions are not part of the current
		// canonical schema. The matching durable transition below the differ
		// accounts for them; treating a building index as already desired would
		// erase the step and make a lost-response retry unobservable.
		if !idx.Ready() {
			continue
		}
	...
	replacedColumns := map[string]bool{}
	for _, step := range out.replacements {
		replacement := step.(ReplaceColumn)
		replacedColumns[replacement.Column] = true
	}
	for key, di := range desiredIdx {
		current, exists := curIdx[key]
		if exists && current.Name != di.Name {
			return tableSteps{}, reject.Inputf(
				"migrate: %s: renaming index %q to %q is not supported", name, current.Name, di.Name)
		}
		rebuild := false
		for _, column := range di.Columns {
			rebuild = rebuild || replacedColumns[column]
		}
		if exists && rebuild {
			out.indexDeletes = append(out.indexDeletes, DeleteIndex{Table: name, Index: current.Name})
	...
	sortSteps(out.adds)
	sortSteps(out.defaultChanges)
	sortSteps(out.indexDeletes)
	sortSteps(out.replacements)
	sortSteps(out.constraints)
...
func orderDeletes(tables []model.Table) []model.Table {
	...
	for len(ordered) < len(tables) {
		...
		if !progressed {
			...
			// in name order rather than deleting steps on the floor.
			for _, t := range tables {
				if !done[t.Name] {
					ordered = append(ordered, t)
				}
