...
// ChangeColumnDefault changes the value applied to future inserts that omit
// the column. Passing nil removes the insert default.
func (db *Service) ChangeColumnDefault(
	ctx context.Context,
	table, column string,
	value *model.Default,
) (model.Table, error) {
	resolved, physical, err := db.column(ctx, table, column)
	if err != nil {
		return model.Table{}, err
	}
	err = db.executeCatalog(ctx, execprogram.Statement{
		Name: "change_column_default", Kind: execprogram.ChangeColumnDefault,
		TableID: resolved.SchemaID, ColumnID: physical.SchemaID,
		InsertDefault: execprogram.TypedDefaultForType(value, physical.Type),
	})
	if err != nil {
		return model.Table{}, err
	}
...
// DeleteIndex logically removes an index; physical entries are reclaimed
// automatically in bounded background transactions.
