...
// queries (03_lir) and rows; it drives the binder, planner, and executor and
...
func Open(store kv.TransactionalKV) *DB {
	return OpenWithOptions(store)
}

// OpenWithOptions attaches to the database with execution-layer options. Most
// applications should use Open; this form exists for embedded deployments and
// resource-constrained test harnesses that need explicit schema-job budgets.
func OpenWithOptions(store kv.TransactionalKV, options ...exec.Option) *DB {
	cat := catalog.New(store)
	return &DB{
		cat: cat,
		eng: exec.New(store, cat, options...),
	}
}

// Close stops this handle's background work. The caller still owns and closes
// the KV store supplied to Open.
