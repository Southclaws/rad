...
func TestCloseWaitsForTransactionsAndRejectsLaterWork(t *testing.T) {
	ctx := context.Background()
	store, err := Open("test-"+t.Name(), "memory:///")
	if err != nil {
		t.Fatal(err)
	}
	txn, err := store.Begin(ctx, kv.Snapshot)
	if err != nil {
		t.Fatal(err)
	}
	...
	select {
	case err := <-done:
		t.Fatalf("close returned while a transaction was active: %v", err)
	...
	case <-time.After(5 * time.Second):
		t.Fatal("close did not finish after transaction release")
	}
	if _, _, err := store.Get(ctx, []byte("after-close")); err == nil {
		t.Fatal("operation after close was accepted")
	}
