...
// DataPosition is an opaque storage commit/snapshot position. Higher layers
// may persist it and test identity, but ordering/retention operations belong
// behind KV capabilities; the concrete backend alone interprets the token.
type DataPosition string

// PositionedTxn is implemented by backends that expose the snapshot position
// assigned when a transaction began. Online schema jobs use it as durable
// provenance, while correctness still relies on transactional fences rather
// than guessing from wall time.
type PositionedTxn interface {
	Txn
	BeginPosition() DataPosition
}
