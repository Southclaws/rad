...
func EvalPred(e bound.Expr, env Env) (lir.TriBool, error) {
	switch x := e.(type) {
	case bound.Binary:
		switch x.Op {
		case lir.OpAnd:
...
func Eval(e bound.Expr, env Env) (lir.Value, error) {
	switch x := e.(type) {
	case bound.Literal:
		return x.V, nil
...
func EvalDatum(e bound.Expr, env Env) (lir.Datum, error) {
	if ref, ok := e.(bound.SlotRef); ok && !ref.T.Kind.Scalar() {
		d, ok := env[ref.Slot]
		if !ok {
...
func triValue(t lir.TriBool) lir.Value {
	...
	default:
		return lir.Null(model.TypeBool)
	}
}

func evalArith(b bound.Binary, env Env) (lir.Value, error) {
	l, err := Eval(b.L, env)
	if err != nil {
		return lir.Value{}, err
	}
	...
	resultType := b.Type().Kind.CatalogType()
	if l.Null || r.Null {
		return lir.Null(resultType), nil
	}

	if resultType == model.TypeInt64 {
...
func evalCast(c bound.Cast, env Env) (lir.Value, error) {
	...
	switch {
	case v.Type == model.TypeInt64 && to == model.TypeFloat64:
		return lir.Float64(float64(v.Int64)), nil
	case v.Type == model.TypeFloat64 && to == model.TypeInt64:
