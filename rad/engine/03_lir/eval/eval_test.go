...
func lit(v lir.Value) bound.Literal { return bound.Literal{V: v} }
...
func pred(t *testing.T, e bound.Expr, env Env) lir.TriBool {
	t.Helper()
...
func TestComparisonsWithNull(t *testing.T) {
	...
	for _, op := range []lir.BinaryOp{lir.OpEq, lir.OpNe, lir.OpLt, lir.OpLte, lir.OpGt, lir.OpGte} {
		if got := pred(t, bound.NewBinary(op, one, null), nil); got != lir.TriUnknown {
			t.Errorf("1 %s NULL = %v, want UNKNOWN", op, got)
		}
		if got := pred(t, bound.NewBinary(op, null, null), nil); got != lir.TriUnknown {
			t.Errorf("NULL %s NULL = %v, want UNKNOWN", op, got)
		}
	}

	notEq := bound.NewUnary(lir.OpNot, bound.NewBinary(lir.OpEq, null, one))
	...
	if pred(t, bound.NewBinary(lir.OpEq, one, one), nil) != lir.TriTrue ||
		pred(t, bound.NewBinary(lir.OpLt, one, lit(lir.Int64(2))), nil) != lir.TriTrue ||
		pred(t, bound.NewBinary(lir.OpGt, one, lit(lir.Int64(2))), nil) != lir.TriFalse {
...
func TestConnectivesK3(t *testing.T) {
	...
	for a, ea := range tv {
		for b, eb := range tv {
			if got := pred(t, bound.NewBinary(lir.OpAnd, ea, eb), nil); got != a.And(b) {
				t.Errorf("%v AND %v = %v", a, b, got)
			}
			if got := pred(t, bound.NewBinary(lir.OpOr, ea, eb), nil); got != a.Or(b) {
				t.Errorf("%v OR %v = %v", a, b, got)
			}
		}
		if got := pred(t, bound.NewUnary(lir.OpNot, ea), nil); got != a.Not() {
...
func TestNullPredicates(t *testing.T) {
	...
	if pred(t, bound.NewUnary(lir.OpIsNull, null), nil) != lir.TriTrue ||
		pred(t, bound.NewUnary(lir.OpIsNull, val), nil) != lir.TriFalse ||
		pred(t, bound.NewUnary(lir.OpIsNotNull, null), nil) != lir.TriFalse ||
		pred(t, bound.NewUnary(lir.OpIsNotNull, val), nil) != lir.TriTrue {
...
func TestNullPredicatesOnNestedSlots(t *testing.T) {
	...
	ref := bound.SlotRef{Slot: 3, Name: "owner", T: rowT}

	present := Env{3: lir.ObjectDatum([]lir.ObjectField{{Name: "id", Datum: lir.ScalarDatum(lir.Text("u1"))}})}
	absent := Env{3: lir.NullDatum()}

	if pred(t, bound.NewUnary(lir.OpIsNull, ref), present) != lir.TriFalse ||
		pred(t, bound.NewUnary(lir.OpIsNull, ref), absent) != lir.TriTrue ||
		pred(t, bound.NewUnary(lir.OpIsNotNull, ref), present) != lir.TriTrue {
...
func TestBoolSlotAsPredicate(t *testing.T) {
	ref := bound.SlotRef{Slot: 7, Name: "done", T: lir.Type{Kind: lir.KindBool, Nullable: true}}
...
func TestScalarSlotRoundTrip(t *testing.T) {
	ref := bound.SlotRef{Slot: 4, Name: "estimate", T: lir.Type{Kind: lir.KindFloat64, Nullable: true}}
...
func TestEvalDatum(t *testing.T) {
	...
	d, err := EvalDatum(bound.SlotRef{Slot: 3, Name: "owner", T: rowT}, env)
	if err != nil || d.Kind != lir.DatumObject {
		t.Fatalf("nested slot datum = %v, %v", d, err)
	}
	d, err = EvalDatum(bound.SlotRef{Slot: 4, Name: "n", T: lir.Type{Kind: lir.KindInt64}}, env)
	if err != nil || d.Kind != lir.DatumScalar || d.Scalar != lir.Int64(9) {
		t.Fatalf("scalar datum = %v, %v", d, err)
	}
	d, err = EvalDatum(bound.NewBinary(lir.OpAdd, lit(lir.Int64(1)), lit(lir.Int64(2))), env)
...
func TestArithmetic(t *testing.T) {
	got, err := Eval(bound.NewBinary(lir.OpAdd, lit(lir.Int64(2)), lit(lir.Int64(3))), nil)
	if err != nil || got != lir.Int64(5) {
		t.Fatalf("2+3 = %v, %v", got, err)
	}
	// Integer division truncates.
	got, _ = Eval(bound.NewBinary(lir.OpDiv, lit(lir.Int64(7)), lit(lir.Int64(2))), nil)
	if got != lir.Int64(3) {
		t.Fatalf("7/2 = %v", got)
	}
	// Mixed operands promote to float.
	got, _ = Eval(bound.NewBinary(lir.OpMul, lit(lir.Int64(2)), lit(lir.Float64(1.5))), nil)
	if got != lir.Float64(3) {
		t.Fatalf("2*1.5 = %v", got)
	}
	// NULL propagates with the result's type.
	got, _ = Eval(bound.NewBinary(lir.OpAdd, lit(lir.Int64(1)), lit(lir.Null(model.TypeInt64))), nil)
	if !got.Null || got.Type != model.TypeInt64 {
		t.Fatalf("1+NULL = %v", got)
	}
	// Division by zero is an error, not infinity.
	if _, err := Eval(bound.NewBinary(lir.OpDiv, lit(lir.Int64(1)), lit(lir.Int64(0))), nil); err == nil {
		t.Fatal("int division by zero succeeded")
	}
	if _, err := Eval(bound.NewBinary(lir.OpDiv, lit(lir.Float64(1)), lit(lir.Float64(0))), nil); err == nil {
		t.Fatal("float division by zero succeeded")
	}
	// Negation.
	got, _ = Eval(bound.NewUnary(lir.OpNegate, lit(lir.Int64(4))), nil)
	if got != lir.Int64(-4) {
		t.Fatalf("-4 = %v", got)
	}
}
...
func TestCastEval(t *testing.T) {
	got, _ := Eval(bound.NewCast(lit(lir.Int64(3)), lir.KindFloat64), nil)
	if got != lir.Float64(3) {
		t.Fatalf("cast int->float = %v", got)
	}
	got, _ = Eval(bound.NewCast(lit(lir.Float64(3.9)), lir.KindInt64), nil)
	if got != lir.Int64(3) {
		t.Fatalf("cast float->int = %v (truncating)", got)
	}
	got, _ = Eval(bound.NewCast(lit(lir.Null(model.TypeInt64)), lir.KindFloat64), nil)
	if !got.Null || got.Type != model.TypeFloat64 {
		t.Fatalf("cast NULL = %v", got)
	}
	if _, err := Eval(bound.NewCast(lit(lir.Text("x")), lir.KindBool), nil); err == nil {
...
func TestCrossingsNeverEvaluate(t *testing.T) {
	s := bound.NewRows("rows", nil, nil)

	if _, err := Eval(bound.Exists{Rel: s}, nil); err == nil || !strings.Contains(err.Error(), "unextracted crossing") {
		t.Fatalf("exists eval = %v", err)
	}
	if _, err := Eval(bound.NewFirst(s), nil); err == nil {
		t.Fatal("First evaluated as a scalar")
	}
	if _, err := Eval(bound.NewArray(s), nil); err == nil {
		t.Fatal("Array evaluated as a scalar")
	}
}

func TestEvalErrors(t *testing.T) {
	if _, err := Eval(bound.SlotRef{Slot: 9, Name: "ghost"}, Env{}); err == nil {
