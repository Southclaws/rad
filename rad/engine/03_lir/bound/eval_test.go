...
func TestBoolSlotAsPredicate(t *testing.T) {
	...
	if pred(t, ref, scalarEnv(map[lir.SlotID]lir.Value{7: lir.Bool(true)})) != lir.TriTrue ||
		...
		pred(t, ref, scalarEnv(map[lir.SlotID]lir.Value{7: lir.Null(catalog.TypeBool)})) != lir.TriUnknown {
...
func TestScalarSlotRoundTrip(t *testing.T) {
	...
	v, err = Eval(ref, scalarEnv(map[lir.SlotID]lir.Value{4: lir.Null(catalog.TypeFloat64)}))
	if err != nil || !v.Null || v.Type != catalog.TypeFloat64 {
...
func TestArithmetic(t *testing.T) {
	...
	got, _ = Eval(NewBinary(lir.OpAdd, lit(lir.Int64(1)), lit(lir.Null(catalog.TypeInt64))), nil)
...
func TestCastEval(t *testing.T) {
	...
	got, _ = Eval(NewCast(lit(lir.Null(catalog.TypeInt64)), lir.KindFloat64), nil)
