...
func triExpr(t lir.TriBool) Expr {
	...
	default:
		return lit(lir.Null(catalog.TypeBool))
	}
}
...
func TestComparisonMatrix(t *testing.T) {
	...
	floats := []float64{-1.5, -0.0, 0.0, 1.5, 2.5}
	...
	nullI := lit(lir.Null(catalog.TypeInt64))
	for _, op := range orderedOps {
...
func TestNullPropagationArithmetic(t *testing.T) {
	for _, op := range []lir.BinaryOp{lir.OpAdd, lir.OpSub, lir.OpMul, lir.OpDiv} {
		gi, _ := Eval(NewBinary(op, lit(lir.Int64(1)), lit(lir.Null(catalog.TypeInt64))), nil)
		if !gi.Null || gi.Type != catalog.TypeInt64 {
			t.Errorf("1 %s NULL(int) = %v, want typed NULL int64", op, gi)
		}
		gi, _ = Eval(NewBinary(op, lit(lir.Null(catalog.TypeInt64)), lit(lir.Int64(1))), nil)
		if !gi.Null {
			t.Errorf("NULL(int) %s 1 = %v, want NULL", op, gi)
		}
		gf, _ := Eval(NewBinary(op, lit(lir.Float64(1)), lit(lir.Null(catalog.TypeFloat64))), nil)
		if !gf.Null || gf.Type != catalog.TypeFloat64 {
			t.Errorf("1.0 %s NULL(float) = %v, want typed NULL float64", op, gf)
		}
	}
	// negate(NULL) is NULL.
	if g, _ := Eval(NewUnary(lir.OpNegate, lit(lir.Null(catalog.TypeInt64))), nil); !g.Null {
