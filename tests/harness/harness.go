// Package harness runs battle tests end to end with no codegen in the loop: a
// real server (in-memory SlateDB, the full HTTP stack) fronted by the real
// client. Planner tests can create compact schemas through direct catalog
// mutation; schema tests use NewSchema and the real desired-schema endpoints.
//
//	d := harness.New(t)
...
// NewSchema boots a schema-managed database. Schema changes must travel
// through SchemaDiff and SchemaMigrate; imperative catalog endpoints are
// rejected, matching a server started with RAD_CATALOG_MODE=schema.
func NewSchema(t *testing.T, options ...exec.Option) *DB {
	t.Helper()
	mode := model.ModeSchema
	return newDB(t, &mode, options...)
}

func newDB(t *testing.T, mode *model.Mode, options ...exec.Option) *DB {
	t.Helper()
	...
	if mode != nil {
		if _, err := cat.InitMode(ctx, *mode); err != nil {
			t.Fatalf("harness: initialise catalog mode %q: %v", *mode, err)
		}
	}
