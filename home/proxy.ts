...
// TODO: translate the landing page into french (ask cheaterman for help)
// spanish, portuguese (wife) russian (ziggi or demian) and maybe more.
export function proxy(request: NextRequest, event: NextFetchEvent) {
  const { pathname } = request.nextUrl;
  if (
    pathname === "/docs" ||
    pathname.startsWith("/docs/") ||
    /^\/(en|fr)\/docs(?:\/|$)/.test(pathname)
  ) {
