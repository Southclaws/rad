...
Applying the plan is the frontend's job (`DB.ApplyMigration` in
`rad/engine/06_frontend/migrate.go`). A desired-state migration is one catalog
PIR program, one serializable transaction, and one schema revision in both
modes. Direct-mode convenience operations remain one revision per individual
operation. Every revision stores the full post-change canonical schema and its
SHA-256 identity under `/rad/catalog/meta/schema_revision/`; logical IDs make
snapshots comparable across renames. Retired logical IDs are rejected if they
appear in a later create, while omitted IDs on direct catalog calls are
allocated above every ID seen in current state or history.
