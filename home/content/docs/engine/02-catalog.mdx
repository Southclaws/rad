...
FK-dependency order, created columns, index deletes, index creates, column deletes,
table deletes, referencing-table-first) without touching the store. Renames are recognized only through
hints — otherwise a rename diffs as delete-plus-create. Indexes are matched
structurally (post-rename columns + uniqueness), not by name, so a column
rename never forces a needless delete-and-backfill. Unsupported changes (column
...
(`DB.Migrate` in rad/engine/06_frontend/migrate.go), one step per transaction — the
