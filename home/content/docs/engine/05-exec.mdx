...
rewrite — and deleting a column simply orphans its field, which
...
service, added columns read defaults or NULL, and a deleted-then-recreated
...
**Backfill** (`backfill.go`): `CreateIndexWithBackfill` registers the index and
