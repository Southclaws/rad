...
query executed ten times against one snapshot must produce deep-equal
datums. Go randomises map iteration per range loop, so in-process
repetition genuinely probes for map-order leaks.
