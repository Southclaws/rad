...
Planning and execution trust bound IR completely. The normative contract —
the grammar, the tagged-union shapes, K3 semantics, determinism rules, and a
behavioural spec for every node — is the LIR schema itself
(`rad/protocol/lir.schema.yaml`, generated to `lir.schema.json`); this
document describes the code that implements it.
...
`Aggregate`, `Order`, `Slice`, and `Ref` (an occurrence of a named binding,
below) — consume relations and produce relations, and the output of every
node is usable as an ordinary input (relational closure).

An **`Expr`** computes exactly one typed datum in some scope: `Literal`,
`Column`, `Unary`, `Binary`, `Cast`. Most are scalar; crossings may produce
rows or arrays. Scalar function calls are deliberately absent until their
...
one row. Root `many` and `Array` crossings always require an explicit order,
and a positive `Slice` offset requires an ordered input. An unordered limit is
allowed only until a later observable boundary requires that order.
...
`Ref{Binding, Scope}` is one occurrence of a named binding — a fresh variable
over the binding's committed value, exposing its output columns under `Scope`
exactly as a scan exposes a table's. It is the one relation node that is a leaf
rather than a tree edge to another node.

`Query{Root, Card, Bindings}` is the root: a relation, its materialisation
mode, and the named relations its `Ref`s observe. `RootCard` ∈ `CardMany`
(array of objects), `CardFirst` (object or null), `CardExactlyOne` (object,
error otherwise), `CardScalar` (single value or null; root arity must be 1 and
cardinality statically at most one). `Bindings map[string]Relation` maps each
binding name to its defining relation — the wire's node-id indirection is
already resolved here.

## Bindings and the relation forest

Most relation nodes are tree edges: each names exactly one input and has
exactly one consumer, so the graph rooted at the query root is a finite tree.
`Bindings` are the one deliberate exception — the single sharing construct —
and they make the query a *forest*: the root tree plus one tree per binding.

A **binding** names a derived relation as a first-class relational value. The
name is syntax; the denotation is semantics. A binding denotes exactly one
statement-local *committed* value — one committed bag under bag semantics — so
a nondeterministic body (a `Slice` with no total order) is resolved once and
every occurrence observes the same choice, exactly as two scans of one table
observe the same snapshot. A binding's public output is its root's declared
output shape; its interior scopes never leak.

A **`Ref`** is one occurrence of a binding: a fresh variable ranging over that
committed value under its own `Scope`, uniformly zero-or-many rows regardless
of the body. A `Ref` is therefore like a `Scan` — a leaf that introduces rows
— not like the operators that consume an `input`, and a determinism-sensitive
consumer brings its own `Order` or `Slice` above it, just as it must above a
scan.

Three namespaces stay separate: binding names, node ids, and scope labels.
Binding bodies may reference other bindings, forming a second acyclic
dependency graph over the forest, and every declared binding must be
referenced at least once. *How* a commitment is discharged — materialised
once, or replayed inline at a lone occurrence — is a planner choice (layer
04); the meaning is identical either way.
...
`NewFilter`, …), and `Inputs()` returns child pointers. Each tree in the
forest is single-consumer; the one place sharing carries meaning is a
`Binding`, and `bound.Query{Root, Card, Slots, Bindings}` holds the bound
bindings in dependency order — each `*Binding` flagging a `PlanSensitive`
body, one whose membership is arbitrary and so must not have its plan diverge
per occurrence. The binder is the only producer.
...
- `tribool_test.go` pins the full K3 truth tables for `And`, `Or`, `Not` —
  the tables the LIR schema states in prose.
