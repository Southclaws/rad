...
Source: `rad/engine/01_kv` (package `kv`), with subpackages `keyenc`, `kvslate`, and
`kvtest`. This is the bottom of the layer stack described in `rad/doc.go` and
expressed by the numbered layout: a package under `rad/engine/NN_*` may import
...
outside a transaction. The catalog's mutation helpers and the executor's
constraint checks are all written against `kv.KV` and get transactionality
for free (see `catalog.Catalog.mutate`, `exec.Engine.Txn`).
...
Writes always run under `SerializableSnapshot`: `catalog.Catalog.mutate` and
