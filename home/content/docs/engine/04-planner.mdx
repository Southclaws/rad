...
(along with `exec:`, `catalog:`, `migrate:`, `rad.schema.yaml:`) and maps the error
...
unordered relation would let the access path leak into results. Root `many`
and `Array` crossings always require an explicit order, and a positive slice
offset requires an ordered input. `Scalar` is a cardinality *assertion* —
single column, statically at most one row — and never accepts ordering as a
substitute; "first scalar" must be spelled `Scalar(Slice₁(Order(...)))`.
...
operand type rules for every operator, cast rules (int64↔float64 only), aggregate
...
comment count). `TestBindForcingQueryGolden` pins
