...
| `--admin-addr <value>` | Administration HTTP listen address; JWT mode requires loopback. |  | `RAD_ADMIN_ADDR` |
...
| `--auth-profile <rfc9068\|compatible>` | JWT validation profile; omission selects rfc9068 in JWT mode. Choices: rfc9068, compatible. |  | `RAD_AUTH_PROFILE` |
| `--auth-query-scopes <value>` | Space-separated OAuth scopes; any listed scope grants query access and omission denies it. |  | `RAD_AUTH_QUERY_SCOPES` |
| `--auth-mutate-scopes <value>` | Space-separated OAuth scopes; any listed scope grants data mutation access and omission denies it. |  | `RAD_AUTH_MUTATE_SCOPES` |
| `--auth-catalog-scopes <value>` | Space-separated OAuth scopes; any listed scope grants catalog mutation access and omission denies it. |  | `RAD_AUTH_CATALOG_SCOPES` |
