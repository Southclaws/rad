...
Run Rad until interrupted. A write instance owns the Slate writer; any
...
| `--planner-mode <structural\|cost>` | Access-path selection mode. Choices: structural, cost. | `structural` | `RAD_PLANNER_MODE` |
...
| `--cache-size-mib <value>` | Total Slate decoded cache capacity in MiB. | `128` | `RAD_CACHE_SIZE_MIB` |
