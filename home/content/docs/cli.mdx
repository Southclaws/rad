...
| `--admin-addr <value>` | Administration HTTP listen address; JWT mode requires an administration scope. |  | `RAD_ADMIN_ADDR` |
| `--auth <none\|jwt>` | Public HTTP authentication mode; JWT requires at least one scope setting. Choices: none, jwt. | `none` | `RAD_AUTH` |
| `--auth-issuer <value>` | Trusted JWT issuer; required with --auth jwt. |  | `RAD_AUTH_ISSUER` |
| `--auth-audience <value>` | Audience that identifies this Rad deployment; required with --auth jwt. |  | `RAD_AUTH_AUDIENCE` |
| `--auth-jwks-url <value>` | Trusted JWKS URL; omission uses OIDC discovery from the issuer. |  | `RAD_AUTH_JWKS_URL` |
| `--auth-profile <rfc9068\|compatible\|cloudflare-access>` | JWT validation profile. Omission selects rfc9068 in JWT mode. Choices: rfc9068, compatible, cloudflare-access. |  | `RAD_AUTH_PROFILE` |
| `--auth-query-scopes <value>` | Space-separated authorization values. Any listed value grants query access. Omission denies access. |  | `RAD_AUTH_QUERY_SCOPES` |
| `--auth-mutate-scopes <value>` | Space-separated authorization values. Any listed value grants data mutation access. Omission denies access. |  | `RAD_AUTH_MUTATE_SCOPES` |
| `--auth-catalog-scopes <value>` | Space-separated authorization values. Any listed value grants catalog mutation access. Omission denies access. |  | `RAD_AUTH_CATALOG_SCOPES` |
| `--auth-admin-scopes <value>` | Space-separated authorization values. Any listed value grants administration access. Omission denies access. |  | `RAD_AUTH_ADMIN_SCOPES` |
...
| `--metrics <true\|false>` | Enable or disable OTEL metrics and the GET /metrics route. Choices: true, false. | `false` | `RAD_METRICS` |
| `--relation-cache-size-mib <value>` | Total memory capacity for materialized relation results. | `128` | `RAD_RELATION_CACHE_SIZE_MIB` |
| `--relation-cache-entries <value>` | Maximum materialized relation result count. | `4096` | `RAD_RELATION_CACHE_ENTRIES` |
| `--relation-cache-max-result-size-mib <value>` | Maximum size of one materialized relation result. | `8` | `RAD_RELATION_CACHE_MAX_RESULT_SIZE_MIB` |
...
| `--slate-commit-durability <durable\|memory>` | Commit acknowledgement point. A process failure before a WAL flush can lose memory commits. Choices: durable, memory. | `durable` | `RAD_SLATE_COMMIT_DURABILITY` |
| `--slate-wal-enabled <true\|false>` | Enable the Slate WAL. Disable it only when process failure can lose unflushed data. Choices: true, false. | `true` | `RAD_SLATE_WAL_ENABLED` |
...
| `-f`, `--file <file>` | Desired declarative schema file. | `rad.schema.yaml` |  |
| `--access-token-file <path>` | File holding the bearer access token; Rad reads it before each request. |  | `RAD_ACCESS_TOKEN_FILE` |
