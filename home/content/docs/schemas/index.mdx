...
Column types are `string`, `int64`, `float64`, `bool`, and `bytes`. Raw bytes
cross LIR and application JSON as canonical padded base64. The built-in
`uuid`, `ulid`, and `xid` formats validate fixed-width bytes and render their
conventional text; unknown formats remain advisory metadata. Their generator
defaults are `uuid_v4()`, `uuid_v7()`, `ulid()`, and `xid()` respectively.
`now_ms()` and `increment()` apply to `int64`. Use `pk`, `unique`, `index`, and
`ref` for common single-column definitions, or the table-level `primary_key`,
`indexes`, and `foreign_keys` forms for compound definitions.

`increment()` does not imply a primary key or uniqueness. Explicit values raise
its durable high-water mark but lower and negative values never rewind it;
deletes do not rewind it either. Failed transactions roll allocation back, so a
value that never commits may be reused. The sequence never wraps or cycles, and
the assignment of values to rows from an unordered input relation is not an
ordering contract.
...
| Change an insert default | Immediate for literals and stateless generators; `increment()` cannot be added, removed, or replaced on an existing column |
