...
`nullable`, `format`, a literal `default`, `uuid()`, `now_ms()`, or
`increment()` where the type permits it. `increment()` is valid on `int64`
columns and generates values from one when a create omits the column. Use `pk`,
`unique`, `index`, and `ref` for common single-column definitions, or the
table-level `primary_key`, `indexes`, and `foreign_keys` forms for compound
definitions. `format` is metadata; it does not validate stored values.
...
| Change an insert default | Immediate for literals, `uuid()`, and `now_ms()`; `increment()` cannot be added, removed, or replaced on an existing column |
