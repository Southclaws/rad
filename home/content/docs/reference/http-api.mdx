...
| `401` | application/problem+json `UnauthenticatedProblem` | A bearer access token is missing or invalid. |
| `403` | application/problem+json `Problem` | The process or access token does not permit the operation. |
...
column must be nullable or carry a literal default. A nullable column
may use a generator default; historical rows remain NULL while new
creates run the generator. On a schema-managed database this operation
is always rejected.
...
**Variants:** `UnauthenticatedProblem`, `ForbiddenProblem`, `InvalidProblem`, `ExecutionFailedProblem`, `NotFoundProblem`, `ConflictProblem`, `InternalProblem`
